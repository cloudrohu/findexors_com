...
# Reference Model (MumbaiCompany Same Structure)


class Refrense(models.Model):
    ...
    assigned_to = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="refrense_assigned"
    )
    ...
    category = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="refrense_categories"
    )
    ...
    locality = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="refrense_localities"
    )
    ...
    sub_locality = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="refrense_sublocalities"
    )
    ...
    project = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="refrense_projects"
    )
    ...
    googlemap_status = models.ForeignKey(
        Googlemap_Status,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="refrense_googlemap_status"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        # ✅ Slug Generate
        if is_new and not self.slug:
            self.slug = f"{slugify(self.refrense_name)}-{self.id}"
            super().save(update_fields=["slug"])
    ...
    def clean(self):
        ...
        if total_links == 0:
            raise ValidationError(
                "Please select Response, Company, Real Estate or Refrense."
            )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.refrense:
            ...
            if has_meeting and has_followup:
                self.refrense.status = "Meeting_FollowUp"

            elif has_meeting:
                self.refrense.status = "Meeting"

            elif has_followup:
                self.refrense.status = "FollowUp"

            else:
                self.refrense.status = "New"
