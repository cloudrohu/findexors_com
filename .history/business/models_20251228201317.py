...
class Company(models.Model):
    ...
    # 👇 CHANGE 1: unique=True add kiya hai
    ...
    def save(self, *args, **kwargs):
        # 👇 CHANGE 2: Auto Remove Space Logic
        if self.contact_no:
            # Spaces ko remove kar dega (e.g., "98 76 54" -> "987654")
...
class Comment(models.Model):
    ...
    created_by = models.ForeignKey(
        User,
        related_name='business_comments_created',
    ...
    updated_by = models.ForeignKey(
        User,
        related_name='business_comments_updated',
        on_delete=models.SET_NULL,
        null=True,
        blank=True
    )
...
class VoiceRecording(models.Model):
    ...
    uploaded_by = models.ForeignKey(
        User,
        related_name='business_voice_uploaded',
        on_delete=models.SET_NULL,
        null=True,
        blank=True
    )
...
class Visit(models.Model):
    ...
    VISIT_TYPE_CHOICES = [
        (f"{i}st Visit" if i == 1 else f"{i}nd Visit" if i == 2 else f"{i}rd Visit" if i == 3 else f"{i}th Visit", f"{i} Visit")
        for i in range(1, 11)
    ]
    # Simplified list generation for visit types to avoid manual errors, or stick to your manual list:
    VISIT_TYPE_CHOICES = [
        ("1st Visit", "1st Visit"),
        ("2nd Visit", "2nd Visit"),
        ("3rd Visit", "3rd Visit"),
...
class SocialLink(models.Model):
    ...
    social_site = models.ForeignKey(SocialSite, on_delete=models.CASCADE, null=True, blank=True) # Fixed Spelling
