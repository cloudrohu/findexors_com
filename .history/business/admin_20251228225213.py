...
class CompanyAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    class Media:
        js = ('//ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js', 'js/admin_dependent_dropdown.js',)

    list_display = ("id", "company_name", "category", "city", "locality", "sub_locality", "contact_no", "status", "is_verified", "is_featured", "assigned_to", "created_at")
    list_filter = ("status", "category", "city", "locality", "sub_locality", "is_verified", "is_featured", "assigned_to")
    search_fields = ("company_name", "contact_no", "city__title", "locality__title", "sub_locality__title")
    readonly_fields = ("slug", "created_at", "updated_at", "logo_preview")

    fieldsets = (
        ("🏢 Company Info", {"fields": ("company_name", "category", "city", "locality", "sub_locality", "address", "description", "logo", "logo_preview")}),
        ("📞 Contact Details", {"fields": ("contact_no", "whatsapp", "email", "website", "google_map")}),
        ("📊 Status & Assignment", {"fields": ("status", "assigned_to", "is_active", "is_verified", "is_featured")}),
        ("🧠 SEO", {"fields": ("slug",)}),
        ("🕒 Audit Info", {"fields": ("created_at", "updated_at")}),
    )
    inlines = [ImagesInline, SocialLinkInline, FaqInline, CommentInline, VoiceRecordingInline, VisitInline, FollowupInline, MeetingInline]
    list_per_page = 20
