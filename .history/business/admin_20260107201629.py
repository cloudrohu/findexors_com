...
class FollowupInlineFormSet(BaseInlineFormSet):
    def clean(self):
        ...
        for form in self.forms:
            ...
            if status in ["New Followup", "Re Followup"]:
                active += 1
...
class MeetingInlineFormSet(BaseInlineFormSet):
    def clean(self):
        ...
        for form in self.forms:
            ...
            status = form.cleaned_data.get('status')
            if status in ["New Meeting", "Re Meeting"]:
                active += 1
...
class CompanyAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    ...
    # ✅ FIXED SEARCH (SAFE VERSION)
    # ⚠️ Agar aap sure nahi ho City/Locality ke field name ka
    search_fields = (
        "company_name",
        "contact_no",
    )

    # 🔥 OPTIONAL (AGAR City/Locality me `name` field hai to ye use karo)
    # search_fields = (
    #     "company_name",
    #     "contact_no",
    #     "city__name",
    ...
    fieldsets = (
        ("🏢 Company Info", {
            "fields": (
                "company_name","contact_no", "whatsapp", "email", "category",
                "city", "locality", "sub_locality", "project",
                "address", "description",
                "logo", "logo_preview"
            )
