...
import tabbed_admin.admin

def fixed_parse_fieldsets_inlines_from_tabs(tabs):
    """
    Ye function library ke original function ko replace karega taaki
    wo Strings (fields) aur Classes (Inlines) me confuse na ho.
    """
    fieldsets = []
    inlines = []
    ...
    for tab_label, tab_content in tabs:
        fields = []
        # Agar tab_content tuple/list nahi hai to skip karein
        if not isinstance(tab_content, (list, tuple)):
            continue

        for item in tab_content:
            # FIX: Pehle check karein ki item String (Field name) hai kya
            if isinstance(item, str):
                fields.append(item)
                continue
            
            # Agar String nahi hai, to Inline check karein
            if hasattr(item, '__name__') and 'Inline' in item.__name__:
                if item not in inlines:
                    inlines.append(item)
            # Fallback: Agar __name__ nahi hai par model attribute hai
            elif hasattr(item, 'model'):
                if item not in inlines:
                    inlines.append(item)
        
        # Agar is tab me fields mile, to unhe fieldsets me add karein
        if fields:
            fieldsets.append((tab_label, {'fields': tuple(fields)}))
            
    return fieldsets, inlines

# Library ke function ko overwrite kar rahe hain (Monkey Patching)
tabbed_admin.admin.parse_fieldsets_inlines_from_tabs = fixed_parse_fieldsets_inlines_from_tabs
...
class CompanyAdmin(AutoUserAdminMixin, TabbedModelAdmin): # 🔥 CHANGED: Inherits TabbedModelAdmin
    ...
    def parse_fieldsets_inlines_from_tabs(self, request, obj=None):
        """
        FIX 1: Handle Strings in Tabs (Attributes Error fix)
        """
    ...
    def get_formatted_tabs(self, request, obj=None):
        """
        FIX 2: Bypass Caching & Fix KeyError
        Ye function library ke caching logic ko replace karta hai.
        """
        tabs = self.get_tabs(request, obj)
        fieldsets, inlines = self.parse_fieldsets_inlines_from_tabs(request, obj)
        # Direct dictionary return karein taaki 'fieldsets' key hamesha mile
        return {'fieldsets': fieldsets, 'inlines': inlines, 'tabs': tabs}


    def parse_fieldsets_inlines_from_tabs(self, request, obj=None):
        """
        FIX: Override library method to handle 'str' fields in Tabs.
        Isse 'str object has no attribute __name__' error solve ho jayega.
        """
        tabs = self.get_tabs(request, obj)
        fieldsets = []
        inlines = []
        ...
        for tab_label, tab_content in tabs:
            fields = []
            # Check karein ki tab content list ya tuple hai
            if not isinstance(tab_content, (list, tuple)):
                continue

            for item in tab_content:
                # 1. Agar item String (Field Name) hai
                if isinstance(item, str):
                    fields.append(item)
                
                # 2. Agar item Inline Class hai (Check __name__)
                elif hasattr(item, '__name__') and 'Inline' in item.__name__:
                     if item not in inlines:
                        inlines.append(item)
                
                # 3. Fallback: Agar item Model class/instance hai
                elif hasattr(item, 'model'):
                     if item not in inlines:
                        inlines.append(item)

            # Fields ko fieldset me add karein
            if fields:
                fieldsets.append((tab_label, {'fields': tuple(fields)}))
