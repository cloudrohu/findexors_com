...
class CompanyAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    # 👇 JS File Link (Correct Path)
    class Media:
        js = (
            '//ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js',
            'js/admin_dependent_dropdown.js', # Ye automatically static/js/ mein dhoondega
        )
    
    # ... baaki code same rahega ...
    list_display = (
        "id", "company_name", "category",
        "city", "locality", "sub_locality", "project", # Project field present hai
        "contact_no", "status",
        # ...
    )
    # ...
    ...
    class Media:
        js = (
            '//ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js', # jQuery Library
            'js/admin_dependent_dropdown.js', # Hamara Logic File
        )
    ...
    search_fields = (
        ...
        "sub_locality__title", "project__title"
    )
    ...
    fieldsets = (
        ...
        ("📞 Contact Details", {
            "fields": (
                "contact_no", "whatsapp", "email", "website", "google_map"
            )
        }),
        ("📊 Status & Assignment", {
            "fields": (
                "status", "assigned_to", "is_active", "is_verified", "is_featured"
            )
        }),
        ("🧠 SEO", {"fields": ("slug",)}),
