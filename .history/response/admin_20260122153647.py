...
class ResponseAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    change_list_template = "admin/response/response/change_list.html"
    list_per_page = 25
    ordering = ("-create_at",)
    preserve_filters = True

    list_display = (
        "mr_id","status","contact_no","business_name","business_category",
        "city","locality","assigned_to","create_at","update_at",
    )

    search_fields = (
        "contact_no","business_name","contact_persone",
        "city__name","locality__name",
    )

    list_filter = (
        "status","business_category","city","locality","assigned_to",
        ContactFilter,
    ...
    fieldsets = (
        ("📞 Contact Info", {"fields": ("status", "assigned_to", "contact_no", "contact_persone")}),
        ("🏢 Business Details", {"fields": ("business_name", "business_category", "requirement_types", "city", "locality")}),
        ("🕒 Audit", {"fields": ("create_at", "update_at", "created_by", "updated_by")}),
    )
