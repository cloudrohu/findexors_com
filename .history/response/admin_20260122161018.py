...
# =====================================================
# ✅ SMART SEARCH HELPERS (GLOBAL)
# =====================================================
def normalize_search(term: str) -> str:
    """MR 001 / MR-001 / mr001 -> MR001"""
    if not term:
        return ""
...
def extract_number(prefix: str, term: str):
    """Extract number from MR001, MT010 etc."""
    if term.startswith(prefix):
        num = term.replace(prefix, "")
...
def digits_only(term: str) -> str:
    """Extract only digits (for phone searches)"""
...
class ResponseAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    ...
    inlines = [MeetingInline, FollowupInline, CommentInline, VoiceInline]
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        term = normalize_search(search_term)
        if not term:
            return queryset, use_distinct

        mr_num = extract_number("MR", term)
        if mr_num is not None:
            queryset = queryset | self.model.objects.filter(id=mr_num)

        phone = digits_only(search_term)
        if phone:
            if len(phone) > 10:
                phone = phone[-10:]
            queryset = queryset | self.model.objects.filter(contact_no__icontains=phone)
...
class MeetingAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        mt_num = extract_number("MT", term)
        if mt_num is not None:
            queryset = queryset | self.model.objects.filter(id=mt_num)

        mr_num = extract_number("MR", term)
        if mr_num is not None:
            queryset = queryset | self.model.objects.filter(response__id=mr_num)

        phone = digits_only(search_term)
        if phone:
            if len(phone) > 10:
                phone = phone[-10:]
            queryset = queryset | self.model.objects.filter(response__contact_no__icontains=phone)
...
class FollowupAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        fu_num = extract_number("FU", term)
        if fu_num is not None:
            queryset = queryset | self.model.objects.filter(id=fu_num)
...
class CommentAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        cm_num = extract_number("CM", term)
        if cm_num is not None:
            queryset = queryset | self.model.objects.filter(id=cm_num)
...
class VoiceRecordingAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        vr_num = extract_number("VR", term)
        if vr_num is not None:
            queryset = queryset | self.model.objects.filter(id=vr_num)
