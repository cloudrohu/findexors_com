...
class ResponseAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    ...
    def action_buttons(self, obj):
        ...
        if obj.contact_no:
            # 📞 Call
            buttons.append(
                f"<a href='tel:+91{obj.contact_no}' style='margin-right:8px;'>📞 Call</a>"
            )

            # 💬 WhatsApp (Ads Acknowledgement)
            name = obj.contact_persone or "Sir/Madam"
            message = f"""
    Hello 👋 {name}
    ...
    """
            whatsapp_url = f"https://wa.me/91{obj.contact_no}?text={quote(message)}"

            buttons.append(
                f"<a href='{whatsapp_url}' target='_blank'>💬 WhatsApp</a>"
            )
