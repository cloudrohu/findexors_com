...
class ResponseAdmin(AutoUserAdminMixin, admin.ModelAdmin):
    ...
    # SPEED FIX: Ek saath saara data load karega (No more slow loading)
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'assigned_to', 'city', 'locality', 'business_category'
        ).prefetch_related('requirement_types', 'meetings', 'followups')

    list_display = (
        "mr_id","status","contact_no","business_name","assigned_to","create_at",
    )

    search_fields = ("contact_no", "business_name", "contact_persone")
    list_filter = (
        "status","business_category","city","assigned_to",
        ContactFilter,
        ("create_at", DateRangeFilter),
    )

    # SEARCH FIX: MR001 format search functionality
    def get_search_results(self, request, queryset, search_term):
        queryset, use_distinct = super().get_search_results(request, queryset, search_term)
        if search_term:
            term = search_term.upper().strip()
            # Agar user MR005 search kare
            if term.startswith("MR"):
                num = term.replace("MR", "").lstrip("0")
                if num.isdigit():
                    queryset |= self.model.objects.filter(id=int(num))
            # Agar user sirf number search kare (ID ya Phone)
            elif term.isdigit():
                queryset |= self.model.objects.filter(id=int(term))
        return queryset, use_distinct

    def mr_id(self, obj):
        return format_html("<b>MR{:03d}</b>", obj.id)
    mr_id.short_description = "ID"
# =====================================================
# ✅ MEETING ADMIN
# =====================================================
